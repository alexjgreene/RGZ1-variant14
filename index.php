...
foreach ($data as $cultivators){
	if (stristr($cultivators['description'],',',true)=="мотоблок"){
	echo '<div class="data">';
	}
	echo '<div class="img">';
	echo	'<img src="'.$cultivators['mainPhoto']['url'].'" width="280">';	
	echo '</div>';
	echo "<h2><b> Общие характеристики: </b></h1>". "<p>". 'КУльтиватор: '  .htmlspecialchars ($cultivators['name']). 'id- '. htmlspecialchars ($cultivators['id']). "<p>".' рейтинг-' .
	htmlspecialchars($cultivators['rating']). "<p>". 'Описание:' .htmlspecialchars ($cultivators['description']). "<p>" .' Идентификатор категории: ' .htmlspecialchars($cultivators['categoryId']). "<p>". 'цена: '.
	number_format($cultivators['prices']['amount'],2,',',' '). " ".htmlspecialchars( $cultivators['prices']['curCode']).
	"<p>". 'Поставщик: '.htmlspecialchars ($cultivators['vendor']).' код поставщика: '.htmlspecialchars($cultivators['vendorId'] ).
	"<p>" .'Количество: '.htmlspecialchars( $cultivators['gradeCount'])."<p> Ссылка на yandex market:" .'<a href='.htmlspecialchars($cultivators['link'])."'>";
	echo $cultivators['link'];
	echo "</a>";
	echo '</div>' ; 	
}
